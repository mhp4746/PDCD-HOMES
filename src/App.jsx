...
export default function App() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const loadedSites = (projectRows || []).map(p => ({
          id: p.id, name: p.name, client: p.client,
          start: p.start_date, finish: p.finish_date, budget: p.budget,
        }));
        ...
        loadedSites.forEach(s => {
          ...
          STAGES.forEach(st => {
            taskMap[s.id][st.id] = {};
            st.tasks.forEach((_, i) => taskMap[s.id][st.id][i] = false);
          });
        });
        const { data: completionRows } = await supabase.from("task_completions").select("*");
        (completionRows || []).forEach(c => {
          if (taskMap[c.project_id]?.[c.stage_id]) {
            taskMap[c.project_id][c.stage_id][c.task_index] = c.completed;
          }
        });
        setTasks(taskMap);

        const { data: tNoteRows } = await supabase.from("task_notes").select("*").order("created_at");
        const tNotes = {};
        (tNoteRows || []).forEach(n => {
          const key = `${n.project_id}-${n.stage_id}-${n.task_index}`;
          if (!tNotes[key]) tNotes[key] = [];
          tNotes[key].push({ text: n.note_text, time: new Date(n.created_at).toLocaleDateString("en-AU",{day:"2-digit",month:"short",year:"numeric"}) + " " + new Date(n.created_at).toLocaleTimeString("en-AU",{hour:"2-digit",minute:"2-digit"}), author: n.author });
        });
        setTaskNotes(tNotes);

        const { data: pNoteRows } = await supabase.from("project_notes").select("*").order("created_at");
        const pNotes = {};
        (pNoteRows || []).forEach(n => {
          if (!pNotes[n.project_id]) pNotes[n.project_id] = [];
          pNotes[n.project_id].push({ text: n.note_text, time: new Date(n.created_at).toLocaleDateString("en-AU",{day:"2-digit",month:"short",year:"numeric"}) + " " + new Date(n.created_at).toLocaleTimeString("en-AU",{hour:"2-digit",minute:"2-digit"}), author: n.author });
        });
  ...
  const toggle = async (siteId, stageId, ti) => {
    const newVal = !tasks[siteId]?.[stageId]?.[ti];
    setTasks(p => ({
      ...p,
      [siteId]: { ...p[siteId], [stageId]: { ...p[siteId][stageId], [ti]: newVal } }
    }));
    if (newVal) {
      await supabase.from("task_completions").upsert({ project_id: siteId, stage_id: stageId, task_index: ti, completed: true }, { onConflict: "project_id,stage_id,task_index" });
    } else {
      await supabase.from("task_completions").delete().eq("project_id", siteId).eq("stage_id", stageId).eq("task_index", ti);
    }
  };
  ...
  const addTaskNote = async () => {
    ...
    const noteObj = { text: noteText.trim(), time: now(), author: "Team" };
    setTaskNotes(p => ({ ...p, [key]: [...(p[key] || []), noteObj] }));
    setNoteText("");
    await supabase.from("task_notes").insert({ project_id: noteModal.siteId, stage_id: noteModal.stageId, task_index: noteModal.taskIdx, note_text: noteText.trim(), author: "Team" });
  };

  const addProjectNote = async (siteId) => {
    if (!projNoteText.trim()) return;
    const noteObj = { text: projNoteText.trim(), time: now(), author: "Team" };
    setProjectNotes(p => ({ ...p, [siteId]: [...(p[siteId] || []), noteObj] }));
    setProjNoteText("");
    await supabase.from("project_notes").insert({ project_id: siteId, note_text: projNoteText.trim(), author: "Team" });
  };

  const addProject = async () => {
    if (!newProject.name.trim()) return;
    const { data, error } = await supabase.from("projects").insert({ name: newProject.name, client: newProject.client, start_date: newProject.start, finish_date: newProject.finish, budget: newProject.budget }).select().single();
    if (error || !data) { console.error("Error adding project:", error); return; }
    const ns = { id: data.id, name: data.name, client: data.client, start: data.start_date, finish: data.finish_date, budget: data.budget };
    setSites(p => [...p, ns]);
    setTasks(p => { const nt = { ...p, [data.id]: {} }; STAGES.forEach(st => { nt[data.id][st.id] = {}; st.tasks.forEach((_, i) => nt[data.id][st.id][i] = false); }); return nt; });
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: BG, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI', sans-serif" }}>
      <div style={{ textAlign: "center" }}>
        <PDCDLogo size={60} />
        <div style={{ marginTop: 16, fontSize: 16, fontWeight: 600, color: TEXT }}>Loading PDCD Tracker...</div>
        <div style={{ marginTop: 8, fontSize: 13, color: TEXT_MUTED }}>Connecting to database</div>
      </div>
    </div>
  );
